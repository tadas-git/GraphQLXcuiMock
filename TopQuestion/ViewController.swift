...
class ViewController: UITableViewController {
	...
	override func viewDidLoad() {
		...
		fetchQuestion()
        gqlFetchAllUsers()
        gqlFetchUserEmailById(id: "W0o1pQBq3h")
	}
...
private extension ViewController {
    ...
    func gqlFetchAllUsers() {
        GraphQLNetwork.shared.apollo.fetch(query: FindAllUsersQuery(), resultHandler: { (result) in
            switch result {
            case .success(let newResult):
                print("success:" + (newResult.data?.users.results[0].username!)!)
    ...
    func gqlFetchUserEmailById(id: String) {
        GraphQLNetwork.shared.apollo.fetch(query: GetEmailQuery(id: id), resultHandler: { (result) in
            switch result {
            case .success(let newResult):
                print("success:" + (newResult.data?.user.emailAddress!)!)
            case .failure(let error):
                print("Error" + error.localizedDescription)
            }
        })
